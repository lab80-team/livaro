...
Issue'lar bu repoda lokal markdown olarak `.scratch/<feature>/` altında tutulur (vault'un `70 Tasks/Task Board`'undan ayrıdır). See `docs/agents/issue-tracker.md`.

### Triage labels

Beş varsayılan triage etiketi olduğu gibi kullanılır: `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`. See `docs/agents/triage-labels.md`.

### Domain docs

Single-context: repo kökünde tek `CONTEXT.md` + `docs/adr/`. See `docs/agents/domain.md`.
