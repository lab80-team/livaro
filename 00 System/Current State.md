...
- **Halı 3D'sinde doku delikleri düzeltildi + saçak ayrı modellendi (2026-08-05/06, main'de `4682b85`)**: krem bordürlü halının dokusunun **%82'si delik açılıyordu** (arka plan silme salt renk yakınlığıyla karar veriyordu; halının açık renkli her yeri beyaz zemine benzediği için siliniyordu) ve delikten dilimin karanlık alt yüzü göründüğü için bordür 3D'de **siyah** çıkıyordu; püskül ise kırpma aşamasında tamamen kesiliyordu. Ürün/zemin ayrımı üç kanıt katmanına çıkarıldı (renk karakteri + iki eşikli bağlantı + topolojik delik doldurma): gövdede delik %82 → **%0**, püskül her senaryoda ayrıştı. Saçak artık ürünün ölçüsünün **dışında** ayrı yassı şerit; modelin gerçek kutusu DB+API+iOS zincirinde taşınıyor. Yüklemede düz ürün fotoğrafı için boşluk zorunlu (birebir kırpılmış katalog görseli istisna). 4 tur Codex incelemesi, 12 bulgu kapatıldı. **499 backend + 84 iOS testi yeşil. Gerçek cihazda AR doğrulaması HENÜZ YAPILMADI.** → [[2026-08-05 Halı ölçüsü saçak hariç — saçak ayrı yassı şerit olarak modellenir]], [[2026-08-06 Düz üründe fotoğraf boşluğu zorunlu — birebir kırpım istisna]], [[2026-08-06 Halı Doku Delikleri, Saçak Modeli ve Migration Kurtarması]].
- **Canlı veritabanında migration bir süre bloke kaldı (6 Ağu, çözüldü)**: elle yazılan migration Prisma model adını tablo adı sanıyordu, canlıda patladı ve başarısız kayıt sonraki tüm migration'ları kilitledi. `migrate resolve --rolled-back` + düzeltilmiş `migrate deploy` ile kurtarıldı; 3 sütun eklendi, 13 kayıt korundu, veri kaybı yok. → [[Known Pitfalls]].
...
- **Halı kalınlık doğrulaması düzeltildi (2026-08-05, main `c6f81fb`, push'landı)**: gerçek bir halı (0,8 cm hav) panelden kaydedilemiyordu — backend koşulsuz ≥ 1 cm dayatıyordu. Sınır artık kategoriye duyarlı (halı ≥ 0,1 cm = 1 mm, mobilya ≥ 1 cm); 3 turlu Codex incelemesi 5 ek bulguyu da kapattı, 443 test yeşil → [[2026-08-05 Halı Kalınlık Doğrulaması Düzeltmesi]], [[Known Pitfalls]].
...
2. **Halı 3D'sini gerçek cihazda AR'da doğrula** — doku delikleri ölçümde kapandı ama telefonda gözle görülmedi; kusur AR'da fark edilmişti.
