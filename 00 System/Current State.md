...
- **Admin panel v1 kapsamı kararlaştı (2026-08-04 thinking session, 2 PM'li tartışmayla) — dilim 1 inşa edildi, kalanı sırada**: ana sayfa pilotta grafik değil iş kuyruğu (4 bekleyen sayacı + son kaydolan mağaza/kullanıcı listeleri; sütun/çizgi grafikler veri birikince); Mağazalar sekmesi kart + 3 türlü kırmızı rozet (ürün onayı + STUCK + düzenleme onayı); Ürünler sekmesi + "yayından kaldırma" (kalıcı silme yalnız yasaklı içerik); yayında yalnız vitrin alanı (foto/başlık/açıklama) değişince yeniden onay; adminde kişi bazlı kullanıcı kartı (tüm bilgiler + kullanım istatistikleri + RoomPlan çıktısı — PM'lerin KVKK itirazı kayıtlı, aydınlatma şartı); başlangıçta 2 yetkili + panelden yetkili ekleme; **işlem günlüğü** (kim/ne zaman/red nedeni) ve **sessiz olay defteri** (4 olay, tek tablo, ekransız — pilottan önce açık, kullanıcı kartı istatistiklerinin veri kaynağı) ikinci turda kabul edildi; ciro/satış/reklam alanları veri doğana dek panelde yok → [[2026 08 04 Thinking Session — Admin Paneli]], [[Admin Panel]]. Açık: foto-3D tutarlılığı, inşa sırası, KVKK aydınlatma metni (avukatla ayarlanacak — kurucu teyidi) → [[Open Questions]].
- **Admin panel v1 dilim 1 inşa edildi (2026-08-05) — main'e HENÜZ birleştirilmedi**: v1'in 8 parçası dilimlere bölündü, kurucu **veri temelini önce** seçti. Dilim 1 = işlem günlüğü + sessiz olay defteri + yeniden onay akışı. Yayındaki üründe başlık/açıklama/fotoğraf değişikliği artık ürüne değil ayrı bir "bekleyen düzenleme" kaydına yazılıyor; **ürün yayında kalıyor ve müşteri onaylı eski halini görüyor**; admin eski|yeni yan yana görüp onaylıyor ya da reddediyor. 572 test, dört derleme kapısı temiz. İnşa sırasında 4 yeni kurucu kararı: yayındaki üründe foto yolu açıldı; onay beklerken ürün yayında kalır + geri yazmak öneriyi iptal eder; **"Onayla + 3D'yi yenile" ÇIKARILDI** (4 Ağustos'ta kabul edilmişti — kod doğrulaması veri kaybı riskini gösterdi); ürün görüntüleme saatte bir kez sayılacak. İncelemeler (görev bazlı + bütünsel + **5 tur Codex**) **on dört gerçek hata** buldu, hepsi planın boşluğuydu — en ciddileri: onay + eşzamanlı foto gönderimi yayındaki ürünün fotoğraflarını kalıcı siliyordu; adminin red notu siliniyordu; admin ekranda gördüğünü değil butona bastığı andaki içeriği onaylıyordu; işlem günlüğü kayıtları hangi ürüne ait olduğu izlenemez haldeydi. **main'e neden girmedi**: Codex kapısı incelemeyi oturumun bulunduğu klasörün HEAD'ine göre kaydediyor, bu oturum vault klasöründen yürütüldüğü için kayıt kod reposundaki commit'e düşmedi — kural gereği atlatılmadı → [[2026-08-05 Build Oturumu — Admin Paneli Dilim 1]], [[Admin Panel]]. **Sırada**: kod reposundan Codex kaydı + merge; sonra Görev 9-10 (edge function'ın mevcut hali yayınlanmalı — 24 Tem'den beri güncellenmemiş, 6 değişiklik bekliyor — sonra olay kaydı eklenip ikinci yayın).
...
1. **Admin dilim 1'in main'e birleştirilmesi** — kod reposunda bir oturumdan Codex incelemesi koşturulup kayıt doğru commit'e düşürülmeli, sonra merge → [[2026-08-05 Build Oturumu — Admin Paneli Dilim 1]] §7.
...
## Son Kararlar
- [[2026-08-05 3D yenileme dilim 1'den çıkarıldı — veri kaybı riski]] (4 Ağustos'ta kabul edilen seçeneği geri alır)
- [[2026-08-05 Yeniden onay davranışı — ürün yayında kalır, geri yazmak öneriyi iptal eder]]
- [[2026-08-05 Yayındaki üründe fotoğraf değiştirme yolu açıldı]]
- [[2026-08-05 Ürün görüntüleme olayı saatte bir kez sayılır]]
